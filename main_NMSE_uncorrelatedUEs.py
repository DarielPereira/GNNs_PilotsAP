...
settings = {
   ...
   'ratioUncorr': [.05, .10, .15, .20, .25, .30]
   # 'ratioUncorr': [.05, .10, .15]
   }

results = {
'Kmeans_basic_positions':[0, 0, 0, 0, 0, 0],
'DCC':[0, 0, 0, 0, 0, 0],
'Kbeams':[0, 0, 0, 0, 0, 0],
'SGPS':[0, 0, 0, 0, 0, 0],
...
for setting in grid_parameters(settings):
    ...
    ratioUncorr = setting['ratioUncorr']

    print(f'Ratio uncorrelated UEs: {ratioUncorr}')
    ...
    for iter in range(nbrOfSetups):
        ...
        gainOverNoisedB, R, pilotIndex, clustering, D, D_small = generateSetup_wUncorrelatedUEs(L, K, N, tau_p, ratioUncorr, ASD_varphi, ASD_theta,
    ...
    results[modes['clustering_mode']][round(ratioUncorr / 0.05 - 1)] = system_NMSEs
...
np.savez(f'./GRAPHs/VARIABLES_SAVED/NMSE_uncorrUEs_5_30_Iter_{nbrOfSetups}',
