...
Performance data is stored so it can be used for creating CDF graphs of the SE (Spectral Efficiency).
"""
...
configuration = {
    ...
    'K': 75,                   # number of UEs
    'T': 100,                       # number of APs connected to each CPU
...
results = {
    'Graph': [np.zeros((K * nbrOfSetups)), np.zeros((K * nbrOfSetups))],
    'BPA':  [np.zeros((K * nbrOfSetups)), np.zeros((K * nbrOfSetups))],
    'I_best':  [np.zeros((K * nbrOfSetups)), np.zeros((K * nbrOfSetups))],
    'ALL':  [np.zeros((K * nbrOfSetups)), np.zeros((K * nbrOfSetups))]
}
...
for heuristic_mode in heuristic_modes:
    ...
    for setup_iter in range(nbrOfSetups):
        ...
        results[heuristic_mode][0][setup_iter*K:(setup_iter+1)*K] = SE.flatten()
        results[heuristic_mode][1][setup_iter * K:(setup_iter + 1) * K] = np.sum(D, axis=0)

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
file_name = f'./GRAPHs_heuristics/VARIABLES_SAVED/SE_CDF_Comb_'+comb_mode+f'_NbrSetups_{nbrOfSetups}_'+timestamp+'.pkl'
